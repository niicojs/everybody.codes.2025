...
for (const line of lines) {
  if (line.startsWith('SWAP ')) {
    ...
    let [rank, value, left, right] = [one.rank, one.value, one.left, one.right];
    [one.rank, one.value, one.left, one.right] = [
      two.rank,
      two.value,
      two.left,
      two.right,
    ];
    [two.rank, two.value, two.left, two.right] = [rank, value, left, right];
  } else {
